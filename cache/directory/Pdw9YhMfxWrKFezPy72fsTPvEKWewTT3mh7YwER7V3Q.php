...
]; ?><div<?= (is_bool($_pug_temp = $pugModule['Phug\\Formatter\\Format\\BasicFormat::attributes_assignment']([], ['style' => 'background-color: #112233; color: #fff;'])) ? var_export($_pug_temp, true) : $_pug_temp) ?>><h2>Debug</h2><pre<?= (is_bool($_pug_temp = $pugModule['Phug\\Formatter\\Format\\BasicFormat::attributes_assignment']([], ['style' => 'font-size: 12px; font-family: serif;'])) ? var_export($_pug_temp, true) : $_pug_temp) ?>><<?= (is_bool($_pug_temp = print_r((isset($info) ? $info : null))) ? var_export($_pug_temp, true) : $_pug_temp) ?>></<?= (is_bool($_pug_temp = print_r((isset($info) ? $info : null))) ? var_export($_pug_temp, true) : $_pug_temp) ?>></pre></div>